...
function renderMetadataObj(row,queue) {
  if(queue === true){
    return {
...
exports.setup = (mstream) => {
 ...
 //   player.addSongWizard(req.body.filepath, {}, true);
  });

  mstream.post('/api/v1/db/to-queue', (req, res) => {
    res.json(this.pullMetaData(req.body.filepath, req.user, true));
 //   player.addSongWizard(req.body.filepath, {}, true);
  });
  ...
  exports.pullMetaData = (filepath, user,queue) => {
    ...
    if (!result || !result[0]) {
      if(queue === true){
        return { "filepath": filepath, "metadata": null, "queue":true };
      }else{
    ...
    if(queue === true){
      return renderMetadataObj(result[0],true);
    }else{
...
/*
http://www.sharepointnadeem.com/2014/11/sharepoint-get-files-from-folder-using.html
*/
  ...
  mstream.get('/api/v1/db/songs', (req, res) => {
    ...
    const results = db.getFileCollection().chain().find({}).compoundsort(['disk','track','filepath']).eqJoin(db.getUserMetadataCollection().chain(), leftFun, rightFunDefault, mapFunDefault).data();
  ...
  mstream.post('/api/v1/db/artist-songs', (req, res) => {
    if (!db.getFileCollection()) { throw new Error('DB Not Working'); }

    const searchClause = [
      renderOrClause(req.user.vpaths),
      {'artist': { '$eq': req.body.artist ? String(req.body.artist) : null }}
    ];
    ...
    const results = db.getFileCollection().chain().find({
      '$and': searchClause
    }).compoundsort(['disk','track','filepath']).eqJoin(db.getUserMetadataCollection().chain(), leftFun).data();
  ...
  mstream.post('/api/v1/db/rate-song', (req, res) => {
    ...
    if (!result2) {
      ...
      const a = db.getUserMetadataCollection().update(result2);
      console.log(a);
      console.log(result2);
