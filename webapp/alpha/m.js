...
function renderFileWithMetadataHtml(filepath, lokiId, metadata) {
  return `<li data-lokiid="${lokiId}" data-playlistid="queue" class="collection-item">
...
async function onBackButton() {
  ...
  } else if (backState.state === 'allSongs') {
    await getAllSongs(undefined);
...
let address = 'http://localhost:19006/';
  console.log(address);
  return `
...
async function loadQueuePlaylist(first) {
  try {
    ...
    document.getElementById('directoryName').innerHTML = 'Last added song';
    document.getElementById('filelist').innerHTML = getLoadingSvg();
    currentBrowsingList = [];
    programState.push({
      state: 'playlist',
...
async function getAllSongs() {
  ...
  programState = [{ state: 'allSongs' }];
  currentBrowsingList = [];

  document.getElementById('localSearchBar').value = '';

  try {
    const response = await MSTREAMAPI.songs()
    //parse through the json array and make an array of corresponding divs
    let files = '<ul class="collection">';
    response.forEach(song => {
      currentBrowsingList.push({ type: 'file', name: song.metadata.title ? song.metadata.title : song.metadata.filename });
